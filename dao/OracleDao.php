<?php
final class OracleDao extends PDOStatement{
    private $db = null;
    public function __destruct() {
        $this->db = null;
    }
    public function find(OracleSearchCriteria $search = null) {
        $result = array();
        foreach ($this->query($this->getFindSql($search)) as $row) {
         ...
         //
            $oracle = new Oracle();
            OracleMapper::map($oracle, $row);
            $result[$oracle->getId()] = $oracle;
        }
        //print_r($result);
        return $result;
    }
    public function find3(OracleSearchCriteria $search = null) {
         $result = array();
         //$sth=$this->query($this->getFindSql3($search));
         //$linhas = $sth->fetchAll(PDO::FETCH_ORI_FIRST);
         //print_r($linhas);echo "<br<br>";
         //$x=1;
       foreach ($this->query($this->getFindSql3($search)) as $row) {
         $oracle = new Oracle();
        //foreach ($linhas as $row) {
         //$row=array_merge($row, array('id'=>$x));
    ...
    private function getFindSql(OracleSearchCriteria $search = null) {
    ...
    private function getFindSql3(OracleSearchCriteria $search = null) {
     //$search->setSINISTRO('0133.93.03.00000314');
     //echo $search->getSINISTRO();die;
        $sql = 'SELECT * FROM SINISTROS_FUP';
        $orderBy = 'COD_SIN';
        $sql .= ' WHERE COD_SIN LIKE \'%'.$search->getSINISTRO().'%\'';
        if ($search !== null) {
    ...
    public function find4(OracleSearchCriteria $search = null) {
